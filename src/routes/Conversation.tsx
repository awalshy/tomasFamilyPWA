...
function Conversation() {
  ...
  if (convId) api.messages.registerListenerToConv(convId, dispatch)
  ...
  useEffect(() => {
    ...
    if (messages.length <= 0 && conversation) dispatch(getMessagesInConv(convId))
    if (!conversation) dispatch(loadConversation(convId))
  ...
  return (
    <PageLayout title={conversation ? conversation.name : 'Conversation'}>
      ...
      >
        ...
        >
          <Typography variant="h5">{conversation?.name || 'Conversation'}</Typography>
        </div>
        <Divider />
        <div
          style={{
