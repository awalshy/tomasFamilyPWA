...
// Async Thunks
export const getMessagesInConv = createAsyncThunk('messages/getConv', async (convId: string) => {
  const api = new API()
  const msgs = await api.messages.getMessagesInConv(convId)
  return msgs
})

export const sendMessage = createAsyncThunk('message/sendMessage', async (message: TMessage) => {
  const api = new API()
...
// Selectors
export const { selectAll: selectAllMessages } = messagesAdapter.getSelectors<RootState>(
