...
export const signUpUserDetails = createAsyncThunk(
  ...
  async ({ firstName, lastName }: { firstName: string; lastName: string }) => {
    ...
    if (!fireUser) throw Error('Not logged In')
...
export const signUpUserFamily = createAsyncThunk(
  ...
  async ({ familyCode }: { familyCode: string }) => {
    ...
    const conv = await firebase
      .firestore()
      .collection(collecs.convs)
      .doc(family.get('convId'))
      .get()
...
export const signOutUser = createAsyncThunk('app/SignOut', async () => {
...
const appSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(signInUser.fulfilled, (state, action) => {
      ...
      state.user = action.payload
      toast.success(
        action.payload?.firstName ? `Bonjour ${action.payload?.firstName}` : 'Connecté !'
      )
    })
