...
const Tile = ({ track, audioPlaying }: ITileProps) => {
  return (
    <Card
      elevation={3}
      style={Object.assign(
        { borderRadius: 12, width: '90%', height: '95%', backgroundColor: 'lightgray' },
        audioPlaying ? { border: '2px solid darkred' } : {}
      )}
    >
      <AgoraVideoPlayer
        className="vid"
  ...
  if (users.length === 1)
    return (
      <div>
        <div style={{ width: '100vw', height: '100vh' }}>
          <Tile
            track={users[0].videoTrack}
            audioPlaying={users[0].audioTrack?.isPlaying || false}
          />
        </div>
        <div
          style={{
  ...
  if (users.length === 2)
    return (
      <div>
        ...
        <div style={{ width: '100vw', height: '50vh' }}>
          <Tile
            track={users[1].videoTrack}
            audioPlaying={users[1].audioTrack?.isPlaying || false}
          />
        </div>
        <div
          style={{
