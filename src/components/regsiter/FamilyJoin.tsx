...
function FamilyJoin() {
  ...
  const registerToFamily = async () => {
    dispatch(signUpUserFamily({ familyCode }))
      .unwrap()
      .then(() => {
        navigate('/')
      })
      ...
      {!res && !loading && (
        <React.Fragment>
          ...
          <div style={{ marginTop: '1vh', display: 'flex', flexDirection: 'row-reverse' }}>
            <Button onClick={check} variant="outlined" color="primary">
              Vérifier
            </Button>
          </div>
        </React.Fragment>
      )}
      {loading && <CircularProgress color="primary" variant="indeterminate" />}
      {res && !err && (
        <React.Fragment>
          ...
          >
            <Button onClick={registerToFamily} variant="outlined" color="primary">
